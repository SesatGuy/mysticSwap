...
// 🚀 Brute Force Protection - Track Failed Logins by IP
...
// Block login attempts if there are too many failed attempts within 10 minutes (600 seconds)
...
if (!isset($_SESSION['admin_logged_in'])) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
        // Use bcrypt with a cost of 10 for password verification
        $bcrypt_cost = 10;
        if ($_POST['username'] === $adminData['username'] && password_verify($_POST['password'], $adminData['password_hash'])) {
            // Successful login, create a session and regenerate CSRF token
            session_regenerate_id(true);
            $_SESSION['admin_logged_in'] = true;
            $_SESSION['failed_attempts'] = 0;
            $_SESSION['csrf_token'] = bin2hex(random_bytes(32)); // Regenerate CSRF token on login
...
// 🚀 Handle Order Actions (CSRF Protected)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['order_id'])) {
    // Validate CSRF token before processing any critical action (e.g., status change, delete)
